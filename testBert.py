...
def prepare_train_test_data_for_bert(year=2023):
    ...
    papers_train = utils.load_json(data_year_dir, "paper_source_trace_train_ans.json")
    papers_valid = utils.load_json(data_year_dir, "paper_source_trace_valid_wo_ans.json")
    papers_test = utils.load_json(data_year_dir, "paper_source_trace_test_wo_ans.json")
    ...
    for file in tqdm(files):
        ...
        for bid in bid_to_title:
            ...
            for label_title in source_titles:
                if fuzz.ratio(cur_ref_title, label_title) >= 60:
                    flag = True
                    cur_pos_bib.add(bid)
    ...
    with open(join("data/PST/", "bib_context_train.txt"), "w", encoding="utf-8") as f:
        for line in x_train:
            f.write(line + "\n")
    
    with open(join("data/PST/", "bib_context_valid.txt"), "w", encoding="utf-8") as f:
        for line in x_valid:
            f.write(line + "\n")
    
    with open(join("data/PST/", "bib_context_test.txt"), "w", encoding="utf-8") as f:
        for line in x_test:
            f.write(line + "\n")
    
    with open(join("data/PST/", "bib_context_train_label.txt"), "w", encoding="utf-8") as f:
        for line in y_train:
            f.write(str(line) + "\n")
    
    with open(join("data/PST/", "bib_context_valid_label.txt"), "w", encoding="utf-8") as f:
        for line in y_valid:
            f.write(str(line) + "\n")
    
    with open(join("data/PST/", "bib_context_test_label.txt"), "w", encoding="utf-8") as f:
        for line in y_test:
            f.write(str(line) + "\n")
...
def train(year=2023, model_name="scibert"):
    ...
    with open(join("data/PST/", "bib_context_train.txt"), "r", encoding="utf-8") as f:
        for line in f:
            train_texts.append(line.strip())
    with open(join("data/PST/", "bib_context_valid.txt"), "r", encoding="utf-8") as f:
        for line in f:
            dev_texts.append(line.strip())

    with open(join("data/PST/", "bib_context_train_label.txt"), "r", encoding="utf-8") as f:
        for line in f:
            train_labels.append(int(line.strip()))
    with open(join("data/PST/", "bib_context_valid_label.txt"), "r", encoding="utf-8") as f:
        for line in f:
    ...
    if model_name == "bert":
        ...
        BERT_MODEL = "allenaiModel/scibert_scivocab_uncased"
...
def train_eval(year=2023, model_name="scibert"):
    ...
    if model_name == "bertModel":
        ...
        BERT_MODEL = "albert-base/albert-base-v2"
    elif model_name == "roberta":
        BERT_MODEL = "roberta/roberta-base"
        #BATCH_SIZE = 12
    elif model_name == "electra":
        BERT_MODEL = "electra-base"
    elif model_name == "deberta":
        BERT_MODEL = "deberta-base"
    elif model_name == "xlnet":
        BERT_MODEL = "xlnet-base"
    elif model_name == "distilbert":
        BERT_MODEL = "distilbert/distilbert-base-uncased"
    else:
        raise NotImplementedError
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        correct_labels += [tensor.detach().cpu().numpy() for tensor in label_ids]
        
        print(str(step)+'/'+str(len(test_dataloader)))
        #break
    sc_list=[]
    for ii in predicted_scores:
        sc=float(utils.sigmoid(ii))
        sc_list.append(sc)
        
    model1=pd.DataFrame()
        
    model1['pre']=sc_list
    model1['labels']=correct_labels
        
    model1.to_csv(model_name+'.csv',index=False)
...
if model_name == "bertModel":
    ...
    BERT_MODEL = "electra/electra-base"
elif model_name == "deberta":
    BERT_MODEL = "deberta-base"
elif model_name == "xlnet":
    BERT_MODEL = "xlnet-base"
...
if model_name=="roberta":
    model.load_state_dict(torch.load(join(settings.OUT_DIR, "kddcup", "robert", "pytorch_model.bin"), map_location=torch.device('cpu')))
elif model_name=="albert":
    model.load_state_dict(torch.load(join(settings.OUT_DIR, "kddcup", "albert-base", "pytorch_model.bin"), map_location=torch.device('cpu')))
elif model_name=="bertModel":
    model.load_state_dict(torch.load(join(settings.OUT_DIR, "kddcup", "bertModel", "pytorch_model.bin"), map_location=torch.device('cpu')))
else:
    model.load_state_dict(torch.load(join(settings.OUT_DIR, "kddcup", model_name, "pytorch_model.bin"), map_location=torch.device('cpu')))
...
def extract_features(items):
    ...
    for item in items:
        feature_vector = [
            # 将 input_ids、input_mask 和 segment_ids 属性转换为特征向量
            item.input_ids,
            item.input_mask,
            item.segment_ids
        ]
...
for paper in tqdm(papers):
    ...
    test_features_vector = extract_features(test_features)

    tt.append(test_features_vector)

    #np.save('test_features_vector.np',tt)
...
utils.dump_json(sub_dict, join('data/PST'), "test_submission_bert.json")
